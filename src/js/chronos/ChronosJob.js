  ...
  properties: {
    ...
    resources: {
      ...
      properties: {
        ...
        gpus: {
          title: "GPUS",
          default: JobResources.DEFAULT_GPU,
          type: "number",
          getter(job) {
            return `${job.getGpus()}`;
          },
          externalValidator({ general }, definition) {
            if (
              ValidatorUtil.isDefined(general.gpus) &&
              !ValidatorUtil.isNumberInRange(general.gpus)
            ) {
              definition.showError = "GPU must be a positive number";
